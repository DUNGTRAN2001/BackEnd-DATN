...
let checkRequiredFields = (inputData)=>{
  let arrFields = ['doctorId','contentHTML','contentMarkdown','action','selectedPrice','selectedPayment','selectedProvince','nameClinic','addressClinic','note','specialtyId','clinicId']
  let isValid = true;
  let element = '';
    ...
    try {
      if (!doctorId) {
        ...
        if (dataDoctor && dataDoctor.image) {
          dataDoctor.image = Buffer.from(dataDoctor.image, "base64").toString(
            "binary"
          );
        }
        if (!dataDoctor) {
...
let bulkCreateSchedule = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.dataTime || !data.doctorId || !data.date) {
        resolve({
          errCode: 1,
          errMessage: "Missing required parameter",
        });
      } else {
        let schedule = data.dataTime;
...
let getExtraInforDotorById = (doctorId) => {
  return new Promise(async (resolve, reject) => {
    try {
      if(!doctorId){
        resolve({
